...
@interface allCityViewController ()<UITableViewDataSource,UITableViewDelegate,PullingRefreshTableViewDelegate,AMapLocationManagerDelegate>{
    NSString *kNearBy;
...
- (void)netWork{
    [self mapLocation];
    kNetWork = [NSString stringWithFormat:@"http://api.breadtrip.com/destination/place/%@/%@/pois/%@/?start=%ld&count=20&sort=default&shift=false&latitude=%.10f&longitude=%.10f",self.typeId,self.userId,_page,(long)_pagecount,_lat,_lon];
